jQuery ($) ->
  ...
  class LocationView extends Backbone.View
    ...
    render: =>
      ...
      @filters.address.val @model.get( 'id' )

    geocode: =>
      @deferred = $.Deferred()

      if @model.get 'id'
        args = 'address': @model.get 'id'
      else
  ...
  class MapCanvasView extends Backbone.View
    ...
    clusterOverlay: (c) =>
      ...
      content = _.map markers, (marker) ->
        _.template( $( '#infoBubbleTemplate' ).html(), marker.meta )
  ...
  class MarkerView extends Backbone.View
    template: $( '#pinTemplate' ).html()
    templateInfoBubble: $( '#infoBubbleTemplate' ).html()
    ...
    renderInfoBubble: =>
      ...
      @map.infobubble.setContent( _.template( @templateInfoBubble, @model.toJSON() ) )
