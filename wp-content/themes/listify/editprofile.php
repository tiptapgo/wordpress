...
function print_my_inline_script() {
	...
	if ( wp_script_is( 'jquery', 'done' ) ) {
		...
		<script type="text/javascript">
			...
			function updateadd(){
				var langcounter1 = ((jQuery('.fieldset-tutor_language').length)/2);
				if(langcounter1 < 10){
					jQuery('.addlan').text('ADD MORE+');
				} else if(langcounter1 == 10){
					jQuery('.addlan').text('Adding more languages is not allowed');
				}
			}
			var langcounter = ((jQuery('.fieldset-tutor_language').length)/2);
			jQuery('.addlan').click(function(e){
				e.preventDefault();
				if(langcounter < 10){
					temp = jQuery('.sourcelan:first').clone();
					temp.find('input').each(function(){
						jQuery(this).val('').css({"border-color":"#ebeef1"});
					})
					temp.find('.removediv').show();
					jQuery('.targetlan').append(temp);
					langcounter++;
					updateadd();
				}
				jQuery('.removebtn').click(function(){
					jQuery(this).parents('.sourcelan').remove();
					langcounter--;
					updateadd();
				});				
			});
			...
			setInterval(function(){
				jQuery('.language-picker').find('.row.sourcelan').each(function(){
					templang = jQuery(this).find('input').val();
					tempprof = jQuery(this).find('select').val();
					if(templang != ''){
						tutorlang += templang + ',';
						tutorlang += tempprof + '|';
					}
				})	
				if (tutorlang.charAt(tutorlang.length - 1) == '|') {
					tutorlang = tutorlang.substr(0, tutorlang.length - 1);
				}				
				jQuery('#tutor_lang').val(tutorlang);
				tutorlang = ''; 
				if(jQuery('#tutor_age').val() != ''){
...
var re1 = /^[789]\d{9}$/i;
			...
			var datereg = /^\d+$/;
			jQuery("#submit-job-form").submit(function(e) {
				window.scrollTo(0, 0);				
				...
				jQuery('#submit-job-form').find('input').each(function(){
					...
					<div class="entry-content">
						...
						<form method="POST" action="<?php echo get_template_directory_uri(); ?>/edit-account.php" method="post" id="submit-job-form" class="job-manager-form" enctype="multipart/form-data">							
							...
							<div class="row">
								<div class="col-md-6 col-lg-6 col-xs-12 col-sm-12">							
									<fieldset class="fieldset-mobile_num">
										...
										<div class="field required-field">
											<input type="tel" class="input-text" <?php if(get_user_meta($userid,'active',true) == 'active' && $phone!=''){ echo "disabled";} ?> name="mobile_num" id="mobile_num" placeholder="<?php echo $phone; ?>" value="<?php echo $phone; ?>" maxlength="10" required />
											<?php if(get_user_meta($userid,'active',true) != 'active' || $phone==''){ ?>	
									...
									<fieldset class="fieldset-tutor_gender">
										...
										<div class="field">
											<select name="tutor_gender" id="tutor_gender" required>
												<option value="Male" <?php if(strtolower($gender)=='male') echo 'selected="selected"' ?> >Male</option>
												<option value="Female"<?php if(strtolower($gender)=='female') echo 'selected="selected"' ?> >Female</option>
											</select>
										</div>
									</fieldset>
								</div>
							...
							<div class="language-picker">
								<label for="tutor_lang">Languages known</label>
								<input type="hidden" name="tutor_lang" id="tutor_lang" value="<?php echo $langdump; ?>">
								<?php
								if($langdump != ''){
									for($i = 0; $i < $langctr; $i++){ ?>
									<div class="row sourcelan">
										<div class="col-md-6 col-lg-6 col-xs-12 col-sm-6 langname">
											<fieldset class="fieldset-tutor_language">
												<div class="field ">
													<input type="text" class="input-text" name="tutor_language[]" value="<?php echo $langArray[$i]; ?>" placeholder="Language" maxlength="">
												</div>
											</fieldset>												
										</div>	
										<div class="col-md-4 col-lg-4 col-xs-12 col-sm-4 langprof">	
											<fieldset class="fieldset-tutor_language">
												<div class="field ">
													<select name="tutor_lang_prof[]">
														<option <?php if($profArray[$i] == "Elementary proficiency"){echo 'selected="selected"';} ?> value="Elementary proficiency">Elementary proficiency</option>
														<option <?php if($profArray[$i] == "Limited working proficiency"){echo 'selected="selected"';} ?> value="Limited working proficiency">Limited working proficiency</option>	
														<option <?php if($profArray[$i] == "Professional working proficiency"){echo 'selected="selected"';} ?> value="Professional working proficiency">Professional working proficiency</option>
														<option <?php if($profArray[$i] == "Full professional proficiency"){echo 'selected="selected"';} ?> value="Full professional proficiency">Full professional proficiency</option>																								
														<option <?php if($profArray[$i] == "Native or bilingual proficiency"){echo 'selected="selected"';} ?> value="Native or bilingual proficiency">Native or bilingual proficiency</option>
													</select>
												</div>
											</fieldset>												
										</div>
										<div class="col-md-2 col-lg-2 col-xs-12 col-sm-2 removediv">
											<span class="fa fa-minus-circle removebtn"></span>										
										</div>
									</div>							
									<?php }
								}
								...
								?>
								<div class="targetlan"></div>
								<div class="row sourcelan">
									<div class="col-md-6 col-lg-6 col-xs-12 col-sm-6 langname">
										<fieldset class="fieldset-tutor_language">
											<div class="field ">
												<input type="text" class="input-text" name="tutor_language[]" value="" placeholder="Language" maxlength="">
											</div>
										</fieldset>												
									</div>	
									<div class="col-md-4 col-lg-4 col-xs-12 col-sm-4 langprof">	
										<fieldset class="fieldset-tutor_language">
											<div class="field ">
												<select name="tutor_lang_prof[]">
													...
													<option selected="selected" value="Professional working proficiency">Professional working proficiency</option>
									...
									<div class="col-md-2 col-lg-2 col-xs-12 col-sm-2 hide removediv">
										<span class="fa fa-minus-circle removebtn"></span>										
									</div>
								</div>
								<div class="row">
									<div class="col-md-4 col-lg-4 col-xs-12 col-sm-12 addmore">								
										<a href="#" class="addlan">ADD MORE+</a>
									</div>
								</div>	
							</div>
							...
							<fieldset class="fieldset-tutor_high_edu" id="address">
								...
								<div class="field ">
									<input type="text" class="input-text" name="tutor_high_edu" id="tutor_high_edu" value="<?php echo $qual; ?>" placeholder="Enter your highest educational qualification" maxlength="">
								</div>
							</fieldset>							
							<fieldset>
