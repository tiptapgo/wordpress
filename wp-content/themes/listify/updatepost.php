...
if($dummyid == '' && isset($_GET['id']) && $_GET['id']!=''){
	$dummyid = $_GET['id'];
}
if($dummyid == ''){
	header("location: ".get_site_url()."/edit-classes/");
}
...
$job_type = get_the_terms( $dummyid, 'job_listing_type' );

$job_description = get_post_field('post_content', $dummyid);

$gender_prf = get_post_meta($dummyid,'_tutor_gender_pref',true);

$job_title = get_post_field('post_title', $dummyid);

$job_category = get_the_terms( $dummyid, 'job_listing_category' );


$catstr = $job_category[0]->term_id;
...
foreach ($parentterms as $term){
	...
	foreach ($childterms as $childterm) {
		if($childterm->term_id == $catstr){
			$job_category_main = $term->term_id;
			$catflag = true;
		}
		if($catflag){
			break;
...
$classtype = get_post_meta($dummyid,'_job_classtype', true);
if ($classtype == ''){
	$classtype = 'regular';
}
$jobhrs = '';
if($classtype == "regular"){
	$jobhrs = get_post_meta($dummyid,'multi_job_hrs', true);
	$monthlyclasses = get_post_meta($dummyid,'_job_monthly_classes', true);	
	$monthlyfees = get_post_meta($dummyid,'_job_monthly_fees', true);	
	$numseats = get_post_meta($dummyid,'_job_no_of_seats', true);	
	$hourly_rate = get_post_meta($dummyid,'_hourly_rate',true);
} else if($classtype == "course"){
	$jobdate = get_post_meta($dummyid,'_job_date', true);	
	$starttime = get_post_meta($dummyid,'_job_start_time', true);	
	$endtime = get_post_meta($dummyid,'_job_end_time', true);	
	$hourly_rate = get_post_meta($dummyid,'_hourly_rate', true);	
	$numseats = get_post_meta($dummyid,'_job_no_of_seats', true);
} else if($classtype == "batch"){
	$jobdate = get_post_meta($dummyid,'_job_date', true);
	$monthlyclasses = get_post_meta($dummyid,'_job_monthly_classes', true);
	$daydump = get_post_meta($dummyid,'_job_day_dump', true);
	$timedump = get_post_meta($dummyid,'_job_time_dump', true);
	$hourly_rate = get_post_meta($dummyid,'_hourly_rate', true);
	$session_duration = get_post_meta($dummyid,'_session_duration', true);
	$monthlyfees = get_post_meta($dummyid,'_job_monthly_fees', true);
	$numseats = get_post_meta($dummyid,'_job_no_of_seats', true);
}
get_header();
...
<div id="content" class="site-content">
							...
							<aside class="widget">
								<div class="row">
									<div class="col-md-8 col-xs-12 col-sm-8 col-lg-8">
										<h2 class="blue-h2"> Edit Your Class</h2>
									</div>
								...
								<form novalidate action="<?php echo get_template_directory_uri(); ?>/updateclass.php" method="post" id="submit-job-form" class="job-manager-form" enctype="multipart/form-data">									<div id="tabs">
									<input type="hidden" id="id" name="id" value="<?php echo $dummyid; ?>">
										...
										<div class="form1">											
											...
											<fieldset class="fieldset-job_title">
												...
												<div class="field col-md-6 col-xs-12 col-md-6 col-xs-12 required-field">
													<input type="text" class="input-text" name="job_title" id="job_title" tabindex="1" placeholder="Your profile headline" value="<?php echo $job_title; ?>" maxlength="" required="">
												</div>
											...
											<fieldset class="fieldset-job_description">
												...
												<div class="field col-md-6 col-xs-12 required-field">
													<textarea cols="20" rows="3" class="input-text" name="job_description" tabindex="2" id="job_description" value="<?php echo $job_description ?>" placeholder="Start typing..." maxlength="" required=""><?php echo $job_description ?></textarea>
												</div>
												<div class="col-md-3 col-xs-12">
											...
											<fieldset class="fieldset-job_classtype">
												...
												<div class="field col-md-9 col-xs-12 required-field">
													<input type="hidden" name="job_classtype" id="job_classtype" value="<?php echo $classtype; ?>">													
													<div class="col-sm-4 col-xs-12 col-md-4 col-lg-4 col-xs-offset-right-1" style="padding-left: 0;">
														<div class="classtypebox <?php if($classtype == 'regular' || $classtype== '') { echo 'active';} ?>" data-type='regular'>
															<div class="text-center head">Regular</div>
															Classes that take place every week like French or Maths for middle school.
														</div>
													</div>
													<div class="col-sm-4 col-xs-12 col-md-4 col-lg-4 col-xs-offset-right-1" style="padding-right: 10px;">	
														<div class="classtypebox <?php if($classtype == 'course') { echo 'active';} ?>" data-type='course'>
													...
													<div class="col-sm-4 col-xs-12 col-md-4 col-lg-4 col-lg-4" style="padding-left: 20px;">
														<div class="classtypebox <?php if($classtype == 'batch') { echo 'active';} ?>" data-type='batch'>
										...
										<div class="form2">
											<div class="regular-type">
												<fieldset class="fieldset-new-job_hours" style="position: relative">
													...
													<div class="field col-md-10 col-xs-12">
														<input type="hidden" name="newjobhrs" id="newjobhrs" value="<?php echo $jobhrs; ?>">
												...
												<fieldset class="fieldset-job_monthly_classes">
													...
													<div class="field col-md-4 col-xs-12 required-field">
														<select name="job_monthly_classes" class="job-manager-category-dropdown" id="job_monthly_classes" value="<?php echo $monthlyclasses; ?>" required=""  placeholder="Number of classes in a month.">
												...
												<fieldset class="fieldset-hourly_rate">
													...
													<div class="field col-md-4 col-xs-12 required-field">
														<input type="text" class="input-text" name="hourly_rate" id="hourly_rate" placeholder="" value="<?php echo $hourly_rate; ?>" maxlength="5" required="">
														<div class="fa fa-money classicon"></div>
													</div>
													<div class="col-md-3 col-xs-12"><small>How much do you wish to charge? (in ₹).</small></div>
													<div class="col-md-2 col-xs-12"></div>
													<div class="col-md-1 col-xs-12">
														<span class="fa fa-check-circle greenicon hide"></span>
													</div>												
												...
												<fieldset class="fieldset-job_monthly_fees">
													...
													<div class="field col-md-4 col-xs-12 required-field">
														<input type="text" class="input-text" name="job_monthly_fees" id="job_monthly_fees" placeholder="" value="<?php echo $monthlyfees; ?>" maxlength="5" required="">
														<div class="fa fa-money classicon"></div>
													</div>
													<div class="col-md-3 col-xs-12"><small>Your monthly fees for this course (in ₹).</small></div>
													<div class="col-md-2 col-xs-12"></div>
													<div class="col-md-1 col-xs-12">
														<span class="fa fa-check-circle greenicon hide"></span>
													</div>												
												...
												<fieldset class="fieldset-job_no_of_seats">
													<div class="col-md-2 col-xs-12 text-right"><label for="job_no_of_seats">No of seats</label></div>
													<div class="field col-md-4 col-xs-12 required-field">													
														<select name="job_no_of_seats" class="job-manager-category-dropdown" id="job_no_of_seats" required="" value="<?php echo $numseats; ?>"  placeholder="No of seats in this course.">
															<option value="">No of seats in this course.</option>
															<option value="1">1</option>
															<option value="2">2</option>
															<option value="3">3</option>
															<option value="4">4</option>
											...
											<div class="course-type hide">
												<fieldset class="fieldset-job_date">
													...
													<div class="field col-md-4 col-xs-12 required-field">
														<input type="text" class="input-text" name="job_date" placeholder="" id="job_date" value="<?php echo $jobdate; ?>" required="">
												...
												<fieldset class="fieldset-job_start_time">
													...
													<div class="field col-md-4 col-xs-12 required-field">
														<input type="text" class="input-text" name="job_start_time" placeholder="" id="job_start_time" value="<?php echo $starttime; ?>" required="">
												...
												<fieldset class="fieldset-job_end_time">
													...
													<div class="field col-md-4 col-xs-12 required-field">
														<input type="text" class="input-text" name="job_end_time" placeholder="" id="job_end_time" value="<?php echo $endtime; ?>" required="">
														<div class="fa fa-clock-o classicon"></div>
													</div>
											...
											<div class="batch-type hide">
												<fieldset class="fieldset-job_date">
													...
													<div class="field col-md-4 col-xs-12 required-field">
														<input type="text" class="input-text" name="job_date" placeholder="" id="job_date1" value="<?php echo $jobdate; ?>" required="">
														<div class="fa fa-calendar classicon"></div>
													</div>
												...
												<fieldset class="fieldset-job_schedule">
													<input type="hidden" value="" id="job_day_dump" name="job_day_dump" value="<?php echo $daydump; ?>">
													<input type="hidden" value="" id="job_time_dump" name="job_time_dump" value="<?php echo $timedump; ?>">
													<div class="row">
														<div class="col-md-12 col-xs-12">
															<div class="col-md-2 col-xs-12 text-right"><label for="job_day">Schedule</label></div>
															<span class="skmaddsource">
																<div class="field col-md-4 col-xs-12 required-field">
																	<select name="job_day" class="job-manager-category-dropdown" id="job_day" required="">
													...
													<span class="skmaddtarget">
														<?php
														$daydumpinit = explode(',', $daydump);
														$timedumpinit = explode(',', $timedump);
														for ($i=1; $i < count($daydumpinit); $i++) {  ?>
														<div class="row">
															<div class="col-xs-12 col-md-12">
																<div class="col-md-2"></div>
																<div class="skmaddtarget<?php echo $i; ?>">
																	<div class="field col-md-4 col-xs-12 required-field">
																		<select name="job_day" class="job-manager-category-dropdown" id="job_day" required="">
																			<option value="">Select a day.</option>
																			<option value="monday">Monday</option>
																	...
																	<div class="field col-md-3 col-xs-12 required-field">
																		<input name="job_time" class="input-text" id="job_time" value="<?php echo $timedumpinit[$i+1]; ?>" required="" placeholder="11:30 AM">
																		<div style="right: 45px" class="fa fa-clock-o classicon"></div>
																	</div>
																</div>
														...
														<?php } ?>	
														<?php for ($i= count($daydumpinit); $i < 7; $i++) {  ?>
														<div class="row">
															<div class="col-xs-12 col-md-12">
																<div class="col-md-2"></div>
																<div class="skmaddtarget<?php echo $i; ?>"></div>
																<div class="col-md-2 col-xs-12"><small></small></div>
																<div class="col-md-1 col-xs-12"></div>
															</div>
														</div>																						
												...
												<fieldset class="fieldset-job_monthly_classes">
													<div class="col-md-2 col-xs-12 text-right"><label for="job_monthly_classes" style="margin-left: -40px;line-height: 0;">Number of classes in a month</label></div>
													<div class="field col-md-4 col-xs-12 required-field">
														<select name="job_monthly_classes" class="job-manager-category-dropdown" id="job_monthly_classes" required="" val="<?php echo $numseats; ?>"  placeholder="Number of classes in a month.">
															<option value="">Number of classes in a month.</option>
															<option value="1">1</option>
															<option value="2">2</option>
															<option value="3">3</option>
												...
												<fieldset class="fieldset-session_duration">
													...
													<div class="field col-md-9 col-xs-12 required-field">
														...
														<input type="hidden" name="session_duration" id="session_duration" value="<?php echo $session_duration; ?>">											
													</div>												
													<div class="col-md-1 col-xs-12">
														<span class="fa fa-check-circle greenicon hide"></span>
													</div>												
												...
												<fieldset class="fieldset-job_no_of_seats">
													<div class="col-md-2 col-xs-12 text-right"><label for="job_no_of_seats">No of seats</label></div>
													<div class="field col-md-4 col-xs-12 required-field">
														<select name="job_no_of_seats" class="job-manager-category-dropdown" id="job_no_of_seats" required=""  placeholder="No of seats in this course.">
															<option value="">No of seats in this course.</option>
															<option value="1">1</option>
															<option value="2">2</option>
															<option value="3">3</option>
											...
											<fieldset class="fieldset-job_type">
												...
												<div class="field col-md-6 col-xs-12 required-field">
													<div class="row radio-row">
														<div class="col-xs-12">
															<input type="radio" <?php if($job_type[0]->term_id == 37) { echo 'checked="checked"';} ?> style="margin-left: 5px; margin-right: 5px; width: auto;" data-meta_key="job_type" class="jmfe-radio input-radio" name="job_type" id="job_type-student-location" value="37">Conducted At Student’s Location
														</div>
													</div>
													<div class="row radio-row">	
														<div class="col-xs-12">
															<input type="radio" <?php if($job_type[0]->term_id == 38) { echo 'checked="checked"';} ?> style="margin-left: 5px; margin-right: 5px; width: auto;" data-meta_key="job_type" class="jmfe-radio input-radio" name="job_type" id="job_type-tutor-location" value="38">Conducted At Tutor’s Location
														</div>
													</div>														
												</div>
												<div class="col-md-3 col-xs-12"></div>
											...
											<fieldset class="fieldset-tutor_gender_pref">
												...
												<fieldset class="fieldset-tutor_gender_pref">
													...
													<div class="field col-md-4 col-xs-12">	
														<select name="tutor_gender_pref" class="job-manager-category-dropdown" id="tutor_gender_pref" required=""  placeholder="No of seats in this course.">
															<option <?php if($gender_prf == "I don't have any Preferences") echo "selected=\"selected\""; ?> value="I don't have any Preferences">I don't have any Preferences</option>
															<option <?php if($gender_prf == "Only Females") echo "selected=\"selected\""; ?> value="Only Females">Only Females</option>
															<option <?php if($gender_prf == "Only Males") echo "selected=\"selected\""; ?> value="Only Males">Only Males</option>
														    <option <?php if($gender_prf == "Only School Students") echo "selected=\"selected\""; ?> value="Only School Students">Only School Students</option>
															<option <?php if($gender_prf == "Only Adults") echo "selected=\"selected\""; ?> value="Only Adults">Only Adults</option>																													
														</select>									
													</div>
												...
												<div class="row text-center">
													<input class="button" type="submit" value="Update Class Info" id="submit" style="background: #77c04b !important; font-size: 18px">
												</div>																												
											</div>
										</div>
									</form>
	...
	<script type="text/javascript">		
		...
		jQuery('.job-btn-session').each(function() {
			if(jQuery(this).data('duration') == jQuery('#session_duration').val()){
				jQuery(this).css({
					"color": "#fff",
					"background": "#3396d1",
		...
		function removelink(){
			jQuery('.skmremovelink').click(function() {
				...
				jQuery('.skmaddtarget' + (skmctr-1)).parent('.col-md-12').find('.skmremovelink').removeClass('hide')
				for (i = 1; i < 7; i++) {
					if (jQuery(".skmaddtarget" + i).children().length) {
						skmctr = i+1;
					}
		...
		jQuery('.skmaddlink').click(function(e) {
			...
			if (skmctr < 7) {
				...
				jQuery('input[name=job_time]').each(function(){
					jQuery(this).timepicker({showPeriod: true, showLeadingZero: true});
				})				
		...
		if(jQuery('#job_classtype').val() == 'batch'){
			var daydumpinit = "<?php echo $daydump; ?>";
			var timedumpinit = "<?php echo $timedump; ?>";		
			daydumpinit = daydumpinit.split(',');
			timedumpinit = timedumpinit.split(',');
			jQuery('.batch-type #job_day').val(daydumpinit[0]);
			jQuery('.batch-type #job_day').trigger("chosen:updated");
			jQuery('.batch-type #job_time').val(timedumpinit[0]);			
			var timectr = 0;
			var dayctr = 0;
			jQuery('input[name=job_time]').each(function(){
				jQuery(this).val(timedumpinit[timectr]);
				timectr++;
			});
			jQuery('.batch-type select[name=job_day]').each(function(){
				jQuery(this).val(daydumpinit[dayctr]);
		...
		jQuery('.'+ jQuery('#job_classtype').val() +'-type').find('.greenicondis').each(function() {
			jQuery(this).addClass('greenicon').removeClass('greenicondis')
		})
		function fixclasstype(){
		...
		jQuery(window).load(function() {
			...
			var m = 0;
			if(jQuery('#job_classtype').val() == "regular"){
				hourdump = jQuery('#newjobhrs').val();
				hourdump = hourdump.split('%');
				for (i = 0; i < 7; i++) {
					if (hourdump[i].indexOf("|") >= 0){
						hourunit = hourdump[i].split('|');
						for (j = 0; j < 4; j++) {
							for (k = 0; k < hourunit.length; k++) {
								if(jQuery("#" + (i+1) + (j+1) + "btn").data('slot') == hourunit[k]){
									jQuery("#" + (i+1) + (j+1) + "btn").addClass('active').css({"color":"#3396d1 !important"}).css({"border-color":"#3396d1"});
								}
							}
						}
					} else{
						for (var m = 0; m < 4; m++) {
							if(jQuery("#" + (i+1) + (m+1) + "btn").data('slot') == hourdump[i]){
								jQuery("#" + (i+1) + (m+1) + "btn").addClass('active').css({"color":"#3396d1 !important"}).css({"border-color":"#3396d1"});
							};
						}
					}
				}
			...
			for (var i = 1; i < 8; i++) {
				for (var j = 1; j < 5; j++) {
					if(jQuery("#" + i + j + "btn").hasClass('active')){
						jobhrsArray[i-1][j-1] = jQuery("#" + i + j + "btn").data('slot');
					}
				}
			}
		...
		jQuery('.subcat<?php echo $job_category_main; ?>').val(<?php echo $catstr; ?>);
		jQuery('.subcat<?php echo $job_category_main; ?>').trigger("chosen:updated");

		jQuery('#job_category_main').val(<?php echo $job_category_main; ?>);
		jQuery('#job_category_main').trigger("chosen:updated");

		if (jQuery('#job_classtype').val() == 'regular') {
			jQuery('.regular-type #job_no_of_seats').val(<?php echo $numseats; ?>);
			jQuery('.regular-type #job_no_of_seats').trigger("chosen:updated");	
			jQuery('.regular-type #job_monthly_classes').val(<?php echo $monthlyclasses; ?>);
			jQuery('.regular-type #job_monthly_classes').trigger("chosen:updated");	
		} else if (jQuery('#job_classtype').val() == 'course') {
			jQuery('.course-type #job_no_of_seats').val(<?php echo $numseats; ?>);
			jQuery('.course-type #job_no_of_seats').trigger("chosen:updated");		
		} else if (jQuery('#job_classtype').val() == 'batch') {
			jQuery('.batch-type #job_monthly_classes').val(<?php echo $monthlyclasses; ?>);
			jQuery('.batch-type #job_monthly_classes').trigger("chosen:updated");
			jQuery('.batch-type #job_no_of_seats').val(<?php echo $numseats; ?>);
			jQuery('.batch-type #job_no_of_seats').trigger("chosen:updated");	
		}

		jQuery('select[name="job_category[]"]').each(function(){
			if((jQuery(this).val()==null ||  jQuery(this).val()=="0") && !jQuery(this).parents('.catwrap').hasClass('hide')){
				jQuery(this).parents('.catwrap').addClass('hide');
			} else if((jQuery(this).val()!=null &&  jQuery(this).val()=="0") && jQuery(this).parents('.catwrap').hasClass('hide')){
				jQuery(this).parents('.catwrap').removeClass('hide');
			}
		})
		...
		jQuery('.form1 .uicont,.head2,.head3,.head4').click(function(e) {
			e.preventDefault();
		...
		jQuery('.form2 .uicont,.head3,.head4').click(function(e) {
			e.preventDefault();
			e.stopImmediatePropagation();
			if (jQuery("#accordion").accordion("option", "active") == 1) {
				...
				if (jQuery('#job_classtype').val() == 'course') {
					...
					if (!validDate.test(jQuery('.course-type #job_date').val())) {
						e.preventDefault();
		...
		jQuery('.form3 .uicont,.head4').click(function(e) {
			e.preventDefault();
			e.stopImmediatePropagation();
 ...
 function calcwidth() {
		...
		setInterval(function() {
			if(jQuery("#job_classtype").val() == 'batch'){
				for (i = 1; i < 7; i++) {
					if (!jQuery('.skmaddtarget' + i).children().length) {
						jQuery('.skmaddtarget' + i).parent('.col-md-12').find('.skmremovelink').addClass('hide')
					}
				}		
			}
			if (review) {
