	...
	function the_custom_field_output_as( $field_slug, $job_id = null, $field_value, $args = array() ) {
		...
		if( is_array( $field_value ) && isset( $args['multiple'] ) && $args['multiple'] > 0 ){
			...
			if( ! empty( $args[ 'output_show_label' ] ) ){
				...
				echo "<strong id=\"jmfe-wrap-{$field_slug}\" class=\"jmfe-custom-field-label\">{$args['label']}:</strong> ";
				echo "</div>";
		...
		if ( $label ) {
			...
			echo "<strong id=\"jmfe-wrap-{$field_slug}\" class=\"jmfe-custom-field-label\">{$label}</strong> ";
		}
