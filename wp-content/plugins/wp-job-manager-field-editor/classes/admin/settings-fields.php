...
class WP_Job_Manager_Field_Editor_Settings_Fields {
	...
	function checkbox_field( $a ) {
		...
		echo "<label><input id=\"{$o['name']}\" type=\"checkbox\" class=\"{$a['class']}\" name=\"{$o['name']}\" value=\"1\"  {$a['attributes']} {$checked} /> {$o['cb_label']} </label>";
		$this->description( $o );
	...
	function backup_field( $a ) {
		...
		echo "<button formmethod=\"GET\" formaction=\"{$url}export.php\" id=\"{$o['name']}\" name=\"button_submit\" value=\"{$o['action']}\" type=\"submit\" class=\"button {$a['class']}\" {$a['attributes']}>{$o['caption']}</button>";
		$this->description( $o );
	...
	function button_field( $a ) {
		...
		echo "<button id=\"{$o['name']}\" name=\"button_submit\" value=\"{$o['action']}\" type=\"submit\" class=\"button {$a['class']}\" {$a['attributes']}>{$o['caption']}</button>";
		$this->description( $o );
	...
	function link_field( $a ) {
		...
		echo "<a id=\"{$o['name']}\" href=\"{$o['href']}\" class=\"{$a['class']}\" {$a['attributes']}>{$o['caption']}</a>";
		$this->description( $o );
	...
	function select_field( $a ) {
		...
		echo "<select id=\"{$o['name']}\" class=\"{$a['class']}\" name=\"{$o['name']}\" {$a['attributes']}>";
	...
	function textarea_field( $a ) {
		...
		echo "<textarea cols=\"50\" rows=\"3\" id=\"{$o['name']}\" class=\"{$a['class']}\" name=\"{$o['name']}\" {$a['attributes']}>";
	...
	function textbox_field( $a ) {
		...
		echo "<input id=\"{$o['name']}\" type=\"text\" class=\"{$a['class']}\" name=\"{$o['name']}\" value=\"{$a['value']}\" {$a['placeholder']} {$a['attributes']} />";
		$this->description( $o );
