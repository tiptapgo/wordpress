...
?>
		<input type="radio" style="margin-left: 5px; margin-right: 5px; width: auto;" data-meta_key="<?php echo $field[ 'meta_key' ]; ?>" class="jmfe-radio jmfe-radio-<?php echo $field[ 'meta_key' ]; ?> input-radio" name="<?php echo esc_attr( isset( $field[ 'name' ] ) ? $field[ 'name' ] : $field['meta_key'] ); ?>" id="<?php echo $field['meta_key'] . '-' . esc_attr( $key ); ?>" value="<?php echo esc_attr( $key ); ?>" <?php if ( isset( $field[ 'value' ] ) || isset( $field[ 'default' ] ) ) checked( isset( $field[ 'value' ] ) ? $field[ 'value' ] : $field[ 'default' ], $key, TRUE ); ?> <?php echo $disabled_option; ?>/><?php echo esc_html( $value ).'</br>'; ?>
<?php
	endforeach;
	if( ! isset( $admin ) || ! $admin ):
?>
