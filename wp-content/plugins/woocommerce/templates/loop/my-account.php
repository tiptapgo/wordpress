...
$stars =  get_post_meta( (int)$listid, 'rating', true );

echo $stars;

//querying db for rating if field is empty
/*
if($stars == ''){
	global $wpdb;
	$query = $wpdb->prepare( "
		...
		AND wpc.comment_post_ID =".(int)$listid."
	...
	if ( ! $total || $votes == 0 ) {
		update_post_meta( (int)$listid, 'rating', 0 );	
	}

	$avg    = $total / $votes;
	$rating = round( round( $avg * 2 ) / 2, 1 );

	update_post_meta( (int)$listid, 'rating', $rating );
}
*/
